...
const cityBillboardCommand = {
 config: {
 name: 'city-billboard',
 description: 'Put someone on a city billboard using their profile picture',
 usage: '@user or reply',
 aliases: ['billboard'],
 category: 'fun',
 role: 0,
 ...
 headerText: 'City Billboard Error',
 headerSymbol: '⚠️',
 headerStyle: 'bold',
 ...
 try {
 const billboardURL = `https://betadash-api-swordslush-production.up.railway.app/city-billboard?userid=${targetID}`;
 const outputPath = path.join(_dirnameFixed, 'cache', `citybillboard${targetID}.jpg`);
 if (!fs.existsSync(path.dirname(outputPath))) fs.mkdirSync(path.dirname(outputPath), { recursive: true });

 const response = await axios.get(billboardURL, { responseType: 'stream' });
 ...
 headerText: 'City Billboard',
 headerSymbol: '🏙️',
 headerStyle: 'bold',
 bodyText: `${name} is now on the city billboard!`,
 bodyStyle: 'sansSerif',
 footerText: '',
 }),
 ...
 console.error('[city-billboard] Error sending image:', err.message);
 }
 }
 }, step.time);
 });
 ...
 console.error('[city-billboard] Error:', error.stack || error.message);
 await api.sendMessage(
 AuroraBetaStyler.styleOutput({
 headerText: 'City Billboard Error',
 headerSymbol: '❌',
...
module.exports = cityBillboardCommand;